...
receiving unexpectedly high attention. The asymmetric K/V precision
strategy (higher precision for keys than values) builds on prior work by
KIVI, LeanKV, and others. Our contribution is a complete system that
implements this strategy within an importance-adaptive framework,
targeting the memory-bandwidth constraints of Apple Silicon. On an 8GB
Mac, FoveatedKV achieves 2.02$\times$ memory compression with 0.995+
cosine fidelity, makes a 7B model 2.3$\times$ faster than standard
inference by reducing memory pressure, and recovers facts lost to
quantization via spike-driven promotion.
\end{abstract}
...
Qwen2.5 (0.5B quality, 7B throughput) and Llama-3.2-1B.
...
\midrule
1K & 1.12\,ms & 0.94\,ms & 1.19$\times$ \\
4K & 2.33\,ms & 1.46\,ms & 1.60$\times$ \\
8K & 4.05\,ms & 2.22\,ms & 1.82$\times$ \\
16K & 7.68\,ms & 3.73\,ms & 2.06$\times$ \\
32K & 15.72\,ms & 6.81\,ms & 2.31$\times$ \\
\bottomrule
\end{tabular}
\caption{Fused kernel speedup over Apple SDPA. Break-even at $\sim$512
tokens; bandwidth advantage grows with context length.}
\end{table}

\subsection{End-to-End: 7B on 8GB}

On memory-constrained hardware, the compressed KV cache reduces memory
pressure, making foveated \emph{faster} than standard:

\begin{table}[h]
\centering
\begin{tabular}{rcccc}
\toprule
Context & Standard (tok/s) & Foveated (tok/s) & Speedup & Mem Saved \\
\midrule
512 & 0.7 & 1.6 & 2.3$\times$ & --- \\
1024 & 0.6 & 1.0 & 1.7$\times$ & 50\,MB \\
2048 & 0.2 & 0.4 & 2.0$\times$ & 101\,MB \\
3072 & --- & 0.3 & --- & 151\,MB \\
\bottomrule
\end{tabular}
\caption{Qwen2.5-7B-Instruct-4bit on M2 8GB. Standard OOMs at
$\sim$3--4K; foveated survives to 3K. Both are swap-bound; foveated wins
by reading less memory.}
\end{table}
...
\paragraph{Model scale.} Quality results are on Qwen2.5-0.5B and
Llama-3.2-1B; throughput results on Qwen2.5-7B. Both Qwen (GQA 8-head,
D=128) and Llama (GQA 8-head, D=64) architectures show consistent
behavior. We have not evaluated on 13B+ models. Prior work on asymmetric
K/V quantization~\cite{leankv, kvadaquant} has validated the approach at
larger scales; our system-level contributions (kernel, promotion) need
separate validation.
...
\paragraph{Hardware.} All results are on Apple M2 8GB. The 7B throughput
numbers are swap-bound and reflect memory-pressure dynamics specific to
this hardware. On machines with more memory (16--32GB), the standard
baseline would be faster, changing the crossover point. The kernel
bandwidth advantage (2.3$\times$ at 32K) is hardware-independent.
...
spike-driven promotion yields a practical system for memory-constrained
Apple Silicon inference. The system achieves 2$\times$ memory compression
with negligible quality loss. On an 8GB Mac, it is faster than standard
inference because reading less memory dominates. The promotion
mechanism---detecting spikes during online softmax at zero overhead and
recovering exact fp16 from NVMe-backed archives---closes the quality gap
for sustained multi-fact retrieval, a capability not present in prior
quantization-only approaches.
