...
throughput on the 0.5B model shows foveated is 0.63--0.81$\times$
standard speed due to Python SDPA interceptor overhead that dominates
on small models. On the actual target scenario---Qwen2.5-7B-Instruct-4bit
on 8GB---foveated is 2--8$\times$ faster (0.6 vs 0.3 tok/s at 512
context; 0.8 vs 0.1 tok/s at 1K) because the 2$\times$ memory
compression keeps more data in physical memory, reducing swap pressure.
The kernel bandwidth advantage (up to 3.34$\times$ at 16K on 7B shapes)
